    ...
    public class LoadFolderButton : MonoBehaviour
{
    ...
    private void LoadResourcesFromFolder(string folderPath)
    {
        ...
        foreach (var filePath in filesToLoad)
        {
            ...
            string fileName = System.IO.Path.GetFileName(filePath).ToLower();            switch (extension)
            {
                case ".pmx":
                case ".pmd":
    ...
    private void RestoreMainPanel()    {        if (sideMenuPanel != null) sideMenuPanel.SetActive(true);
