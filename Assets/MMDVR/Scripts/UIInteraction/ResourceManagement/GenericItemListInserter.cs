...
using MMDVR.Scripts.Model; // for CameraData and other data types
using MMDVR.Scripts.UIInteraction.ResourceManagement.ListController; // for the resource controllers
        ...
        else if (listContainer.name.Contains("Camera"))
        {
            if (CameraListController.Instance != null)
            {
                ...
                dummyDraggable.Data = new CameraData { id = "dummy", displayName = "dummy", filePath = "", isFreeCamera = false };
                  CameraListController.Instance.HandleDropOnListArea(dummyGO);                DestroyImmediate(dummyGO);
            }
        }
    }
}
